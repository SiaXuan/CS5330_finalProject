# 概念：shape（张量的形状）

shape 就是"这块数据有几层、每层几个"，用一个括号里的数字列表表示。

## 从一维开始

```python
[1, 2, 3, 4, 5, 6, 7]
# shape = (7,)   ← 一维，7 个元素
```

## 二维：表格 / 矩阵

```python
[[1, 2, 3, 4],
 [5, 6, 7, 8],
 [9, 0, 1, 2]]
# shape = (3, 4)  ← 3 行 4 列
```

读法：**括号里从左到右，依次是"最外层有几个"、"次外层有几个"……**

shape 里每个位置对应一个 **axis（轴）编号**，从 0 开始：

```python
[[1, 2, 3],
 [4, 5, 6]]
# shape = (2, 3)
#          ↑  ↑
#      axis=0  axis=1
#      （行）  （列）
```

`axis=0` 是行方向（上下），`axis=1` 是列方向（左右）。  
`axis=-1` 永远指最后一个维度，二维时等价于 `axis=1`。

对某个 axis 做运算，就是沿那个方向"折叠"，那个维度消失：

```python
a.sum(axis=0)   # 每列求和，行消失 → shape (3,)
a.sum(axis=1)   # 每行求和，列消失 → shape (2,)
```

## 本项目里遇到的 shape

| shape | 含义 |
|-------|------|
| `(512,)` | 512 个数，一个向量 |
| `(1, 512)` | 1 行 512 列，一张图片的向量（包了 batch 层） |
| `(N, 512)` | N 行 512 列，N 张图片各自的向量 → gallery embeddings |
| `(N, G)` | N 条 query × G 张 gallery 图片的相似度分数矩阵 |

## 为什么 (1, 512) 不是 (512,)

CLIP 处理单张图片时，输出其实是 `(1, 512)`，因为 PyTorch 里的模型习惯批量处理，
哪怕只有一张图，也要包一层"batch 维"表示"这是 1 个样本的 batch"。

`unsqueeze(0)` 就是手动加上这一层：`(512,) → (1, 512)`。

→ 详见 [concepts/batch.md](batch.md)

## 底层不是 list

Tensor 底层是**一块连续内存**，数字一个接一个排着：

```
[[1, 2, 3],
 [4, 5, 6]]

内存里实际：[1, 2, 3, 4, 5, 6]
```

shape `(2, 3)` 只是一个读取规则——告诉你每 3 个数算一行。
换个 shape 不移动任何数据，只换规则。

Python list 是指针数组，元素随机分散在内存里，做数学运算要一个个跳着读，很慢。
Tensor 连续存储，CPU/GPU 可以批量并行计算，快很多。

## 怎么查某个变量的 shape

```python
import numpy as np
import torch

a = np.array([[1, 2, 3], [4, 5, 6]])
print(a.shape)   # (2, 3)

t = torch.tensor([[1, 2, 3], [4, 5, 6]])
print(t.shape)   # torch.Size([2, 3])
```
